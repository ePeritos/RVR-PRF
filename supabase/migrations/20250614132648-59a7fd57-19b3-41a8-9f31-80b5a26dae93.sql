-- Fix 1: Ensure triggers exist and are active
DROP TRIGGER IF EXISTS trigger_manutencao_ambientes_nota ON public.manutencao_ambientes;
DROP TRIGGER IF EXISTS trigger_dados_caip_nota_global ON public.dados_caip;

-- Recreate trigger for maintenance score calculation
...
-- Recreate trigger for global score calculation  
CREATE TRIGGER trigger_dados_caip_nota_global
    BEFORE UPDATE ON public.dados_caip
    FOR EACH ROW
    EXECUTE FUNCTION public.trigger_atualizar_nota_global();

-- Fix 2: Add missing environment name mappings to the calculation function
...
BEGIN
    ...
    LOOP
        ...
        BEGIN
            -- Mapear nome do ambiente para campo do formulário (incluindo variações)
            CASE ambiente_record.nome_ambiente
                ...
                WHEN 'Lavabo para servidores' THEN
                    ambiente_selecionado := v_dados_caip.lavabo_para_servidores_sem_box_para_chuveiro = 'Sim';
                ...
                WHEN 'Plataforma para fiscalização de veículos' THEN
                    ambiente_selecionado := v_dados_caip.plataforma_para_fiscalizacao_da_parte_superior_dos_veiculos = 'Sim';
...
-- Fix 3: Ensure the global calculation function works correctly
