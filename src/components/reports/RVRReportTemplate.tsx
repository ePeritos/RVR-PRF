...
export function RVRReportTemplate({ data, className = "" }: RVRReportTemplateProps) {
  ...
  // Extrair sigla da unidade gestora para o solicitante
  const getSiglaUnidade = (unidadeGestora?: string) => {
    if (!unidadeGestora) return 'PRF/XX';
    
    // Extrair sigla - tentar pegar as primeiras letras maiúsculas ou a parte antes de "/"
    const match = unidadeGestora.match(/^([A-Z]+)/);
    if (match) {
      return match[1];
    }
    
    // Se não encontrar, usar formato padrão
    const uf = unidadeGestora.match(/([A-Z]{2})$/);
    return uf ? `PRF/${uf[1]}` : 'PRF/XX';
  };

  // Extrair UF da unidade gestora
  ...
  const solicitante = getSiglaUnidade(data.unidadeGestora);

  return (
    <div className={`bg-white text-black max-w-5xl mx-auto text-sm leading-relaxed ${className}`} id={`rvr-report-${data.id}`} style={{ pageBreakInside: 'avoid' }}>
      <div className="p-8">
        {/* Cabeçalho Oficial */}
      ...
      <div className="p-8">
        ...
        <section className="mb-8" style={{ pageBreakInside: 'avoid' }}>
          ...
          <div className="text-xs space-y-4">
            ...
            <div className="mt-12 text-center" style={{ marginTop: '80px' }}>
        ...
        {/* Rodapé */}
        <div className="border-t border-gray-400 pt-4 mt-8" style={{ marginBottom: '40px' }}>
